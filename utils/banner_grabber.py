...
class BannerGrabber:
    ...
    def grab_banner(self, host: str, port: int) -> Optional[str]:
        ...
        try:
            ...
            # Try to receive initial banner (for services that send on connect)
            banner_data = b""
            try:
                banner_data = sock.recv(1024)
            except socket.timeout:
                # Some services don't send banner immediately, that's okay
                pass
            
            # Send probe if configured and no initial banner
            probe = self._get_probe_for_port(port)
            if probe is not None and len(banner_data) == 0:
                try:
                    sock.send(probe)
                    # Wait for response
                    banner_data = sock.recv(1024)
